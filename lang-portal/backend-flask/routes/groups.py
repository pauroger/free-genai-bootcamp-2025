...
def load(app):
  ...
  def get_group_words(id):
    try:
      ...
      valid_columns = ['german', 'english', 'correct_count', 'wrong_count']
      if sort_by not in valid_columns:
        sort_by = 'german'
      if order not in ['asc', 'desc']:
        order = 'asc'
  ...
  def get_group_words_raw(id):
    try:
      ...
      # Optional: set up pagination parameters
      page = int(request.args.get('page', 1))
      words_per_page = int(request.args.get('limit', 100))  # default limit
      offset = (page - 1) * words_per_page

      # Query to fetch raw word data for the group
      cursor.execute('''
        SELECT w.*
        FROM words w
        JOIN word_groups wg ON w.id = wg.word_id
      ...
      words = cursor.fetchall()
      # Convert each row to a dict (adjust according to your cursor's row format)
      words_data = [dict(word) for word in words]
